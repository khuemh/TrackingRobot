...
const uint8_t cam_tilt_MAX = 160;
const uint8_t cam_tilt_MIN = 30;
uint16_t servoPos;
uint8_t cam_tilt_flag = 0;
...
void leftMotorClkWise(uint16_t speed)
{
    GPIO_SetBits(left_IN1_GPIO_Port, left_IN1_Pin);      // left_IN1 HIGH
    GPIO_ResetBits(left_IN2_GPIO_Port, left_IN2_Pin);    // left_IN2 LOW
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, speed);
}

void leftMotorCounterClkWise(uint16_t speed)
{
	GPIO_SetBits(left_IN1_GPIO_Port, left_IN2_Pin);      // left_IN1 HIGH
    GPIO_ResetBits(left_IN2_GPIO_Port, left_IN1_Pin);    // left_IN2 LOW
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, speed);
}

void leftMotorStop()
{
    GPIO_ResetBits(left_IN1_GPIO_Port, left_IN1_Pin);    // left_IN1 LOW
    GPIO_ResetBits(left_IN2_GPIO_Port, left_IN2_Pin);    // left_IN2 LOW
}
...
void rightMotorClkWise(uint16_t speed)
{
    GPIO_SetBits(right_IN1_GPIO_Port, right_IN1_Pin);      // left_IN1 HIGH
    GPIO_ResetBits(right_IN2_GPIO_Port, right_IN2_Pin);    // left_IN2 LOW
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, speed);
}

void rightMotorCounterClkWise(uint16_t speed)
{
    GPIO_SetBits(right_IN1_GPIO_Port, right_IN2_Pin);      // left_IN1 HIGH
    GPIO_ResetBits(right_IN2_GPIO_Port, right_IN1_Pin);    // left_IN2 LOW
    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, speed);
}

void rightMotorStop(void)
{
    GPIO_ResetBits(right_IN1_GPIO_Port, right_IN1_Pin);    // left_IN1 LOW
    GPIO_ResetBits(right_IN2_GPIO_Port, right_IN2_Pin);    // left_IN2 LOW
}
...
void cam_tilt(uint16_t angle)
{
	if (angle > cam_tilt_MAX) 
  {
    angle = 160;
  }
  else if (angle < cam_tilt_MIN)
  {
    angle = 30;
  }
